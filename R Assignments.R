#Que No.1: 
#Try to code for printing sequence of number of 1 to 50 with difference of 3,5,10.
...
#Que No.3:
#Create data frame with three columns and five rows and write a code to fetch and delete row and column using index and add new column and row to existed dataframe.
...
#Que No.4:
#Write nested if else statement to print weather the number is negative, positive, or zero.
...
#Que No.5:
#Write a program to input any value and check weather it is char,numric, or special char.
...
#Que No.7: 
#Write a program to print given vector in reverse format.
...
#Que No.8: 
#Write a program to get the mode value of given vector.
...
#Que No.9: 
#Write a function to filter any data belongs to setosa in species of iris dataset.
...
#Que No.10:
#Create new column for iris dataset.

iris%>%rowwise()%>%mutate(Mean=mean(c(Sepal.Length, Sepal.Width, Petal.Length, Petal.Width)))

#Que No.11:
#Filter data for versicolor and get only scapel length and  scapel width.
